...
Class Action {
	...
	function submit_reply(){
		...
		foreach($_POST as $k => $v){
			if(!in_array($k, array('id')) && !is_numeric($k)){
				if($k == 'reply') { $v = htmlentities(str_replace("'","&#x2019;",$v)); }
				if(empty($data)) {
					$data .= " $k='$v' ";
				} else {
					$data .= ", $k='$v' ";
		...
		// $data .= ", productivity_id='".$_POST['$productivity_id']."'";
		$data .= ", date=NOW() ";
		// echo $data;exit;
		// if(empty($id)){
		$save = $this->db->query("INSERT INTO user_productivity_replies SET $data");
		// }else{
		// 	$save = $this->db->query("UPDATE user_productivity_replies set $data where id=0");
		// }
		if($save) { return 1; }
		// if($save){
		// 	return 1;
